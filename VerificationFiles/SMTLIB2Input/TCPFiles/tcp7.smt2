(set-logic QF_UFLIA)
(set-option :produce-models true)
(declare-fun readyToSend (Int) Int)
(declare-fun msgBuffer (Int) Int)
(declare-fun readyToReceive (Int) Int)
(declare-fun messageReceived (Int) Int)
(declare-fun ackSent (Int) Int)
(declare-fun ackBuffer (Int) Int)
(declare-fun ackReceived (Int) Int)
(declare-fun waitForAck (Int) Int)
(declare-fun k () Int)
(assert (= (readyToSend 0) 1))
(assert (= (readyToReceive 0) 1))
(assert (= (msgBuffer 0) 0))
(assert (= (messageReceived 0) 0))
(assert (= (ackSent 0) 0))
(assert (= (ackBuffer 0) 0))
(assert (= (ackReceived 0) 0))
(assert (= (waitForAck 0) 0))
(assert (= (ackReceived 7) 1))
(assert (= k 7))
(assert  (ite (= (readyToSend (- 1 1)) 1) 
(and
(= (waitForAck 1) 1) 
(= (msgBuffer 1) 1) 
(= (readyToSend 1) 1) 
(= (messageReceived 1) (messageReceived (- 1 1))) 
(= (ackSent 1) (ackSent (- 1 1))) 
(= (ackBuffer 1) (ackBuffer (- 1 1))) 
(= (ackReceived 1) (ackReceived (- 1 1))) 
(= (readyToReceive 1) (readyToReceive (- 1 1))) 
)
 (ite (and (= (msgBuffer (- 1 1)) 1)  (= (readyToReceive (- 1 1)) 1)) 
(and
(= (msgBuffer 1) 1) 
(= (readyToReceive 1) 1) 
(= (messageReceived 1) 1) 
(= (waitForAck 1) (waitForAck (- 1 1))) 
(= (ackSent 1) (ackSent (- 1 1))) 
(= (ackBuffer 1) (ackBuffer (- 1 1))) 
(= (ackReceived 1) (ackReceived (- 1 1))) 
(= (readyToSend 1) (readyToSend (- 1 1))) 
)
 (ite (= (messageReceived (- 1 1)) 1) 
(and
(= (messageReceived 1) 1) 
(= (ackSent 1) 1) 
(= (ackBuffer 1) 1) 
(= (waitForAck 1) (waitForAck (- 1 1))) 
(= (msgBuffer 1) (msgBuffer (- 1 1))) 
(= (readyToReceive 1) (readyToReceive (- 1 1))) 
(= (ackReceived 1) (ackReceived (- 1 1))) 
(= (readyToSend 1) (readyToSend (- 1 1))) 
)
 (ite (and (= (waitForAck (- 1 1)) 1)  (= (ackBuffer (- 1 1)) 1)) 
(and
(= (waitForAck 1) 1) 
(= (ackBuffer 1) 1) 
(= (ackReceived 1) 1) 
(= (messageReceived 1) (messageReceived (- 1 1))) 
(= (msgBuffer 1) (msgBuffer (- 1 1))) 
(= (readyToReceive 1) (readyToReceive (- 1 1))) 
(= (ackSent 1) (ackSent (- 1 1))) 
(= (readyToSend 1) (readyToSend (- 1 1))) 
)
 (ite (= (ackSent (- 1 1)) 1) 
(and
(= (ackSent 1) 1) 
(= (readyToReceive 1) 1) 
(= (waitForAck 1) (waitForAck (- 1 1))) 
(= (messageReceived 1) (messageReceived (- 1 1))) 
(= (msgBuffer 1) (msgBuffer (- 1 1))) 
(= (ackReceived 1) (ackReceived (- 1 1))) 
(= (ackBuffer 1) (ackBuffer (- 1 1))) 
(= (readyToSend 1) (readyToSend (- 1 1))) 
)
 (ite (= (ackReceived (- 1 1)) 1) 
(and
(= (ackReceived 1) 1) 
(= (readyToSend 1) 1) 
(= (waitForAck 1) (waitForAck (- 1 1))) 
(= (messageReceived 1) (messageReceived (- 1 1))) 
(= (msgBuffer 1) (msgBuffer (- 1 1))) 
(= (ackSent 1) (ackSent (- 1 1))) 
(= (ackBuffer 1) (ackBuffer (- 1 1))) 
(= (readyToReceive 1) (readyToReceive (- 1 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 2 1)) 1) 
(and
(= (waitForAck 2) 1) 
(= (msgBuffer 2) 1) 
(= (readyToSend 2) 1) 
(= (messageReceived 2) (messageReceived (- 2 1))) 
(= (ackSent 2) (ackSent (- 2 1))) 
(= (ackBuffer 2) (ackBuffer (- 2 1))) 
(= (ackReceived 2) (ackReceived (- 2 1))) 
(= (readyToReceive 2) (readyToReceive (- 2 1))) 
)
 (ite (and (= (msgBuffer (- 2 1)) 1)  (= (readyToReceive (- 2 1)) 1)) 
(and
(= (msgBuffer 2) 1) 
(= (readyToReceive 2) 1) 
(= (messageReceived 2) 1) 
(= (waitForAck 2) (waitForAck (- 2 1))) 
(= (ackSent 2) (ackSent (- 2 1))) 
(= (ackBuffer 2) (ackBuffer (- 2 1))) 
(= (ackReceived 2) (ackReceived (- 2 1))) 
(= (readyToSend 2) (readyToSend (- 2 1))) 
)
 (ite (= (messageReceived (- 2 1)) 1) 
(and
(= (messageReceived 2) 1) 
(= (ackSent 2) 1) 
(= (ackBuffer 2) 1) 
(= (waitForAck 2) (waitForAck (- 2 1))) 
(= (msgBuffer 2) (msgBuffer (- 2 1))) 
(= (readyToReceive 2) (readyToReceive (- 2 1))) 
(= (ackReceived 2) (ackReceived (- 2 1))) 
(= (readyToSend 2) (readyToSend (- 2 1))) 
)
 (ite (and (= (waitForAck (- 2 1)) 1)  (= (ackBuffer (- 2 1)) 1)) 
(and
(= (waitForAck 2) 1) 
(= (ackBuffer 2) 1) 
(= (ackReceived 2) 1) 
(= (messageReceived 2) (messageReceived (- 2 1))) 
(= (msgBuffer 2) (msgBuffer (- 2 1))) 
(= (readyToReceive 2) (readyToReceive (- 2 1))) 
(= (ackSent 2) (ackSent (- 2 1))) 
(= (readyToSend 2) (readyToSend (- 2 1))) 
)
 (ite (= (ackSent (- 2 1)) 1) 
(and
(= (ackSent 2) 1) 
(= (readyToReceive 2) 1) 
(= (waitForAck 2) (waitForAck (- 2 1))) 
(= (messageReceived 2) (messageReceived (- 2 1))) 
(= (msgBuffer 2) (msgBuffer (- 2 1))) 
(= (ackReceived 2) (ackReceived (- 2 1))) 
(= (ackBuffer 2) (ackBuffer (- 2 1))) 
(= (readyToSend 2) (readyToSend (- 2 1))) 
)
 (ite (= (ackReceived (- 2 1)) 1) 
(and
(= (ackReceived 2) 1) 
(= (readyToSend 2) 1) 
(= (waitForAck 2) (waitForAck (- 2 1))) 
(= (messageReceived 2) (messageReceived (- 2 1))) 
(= (msgBuffer 2) (msgBuffer (- 2 1))) 
(= (ackSent 2) (ackSent (- 2 1))) 
(= (ackBuffer 2) (ackBuffer (- 2 1))) 
(= (readyToReceive 2) (readyToReceive (- 2 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 3 1)) 1) 
(and
(= (waitForAck 3) 1) 
(= (msgBuffer 3) 1) 
(= (readyToSend 3) 1) 
(= (messageReceived 3) (messageReceived (- 3 1))) 
(= (ackSent 3) (ackSent (- 3 1))) 
(= (ackBuffer 3) (ackBuffer (- 3 1))) 
(= (ackReceived 3) (ackReceived (- 3 1))) 
(= (readyToReceive 3) (readyToReceive (- 3 1))) 
)
 (ite (and (= (msgBuffer (- 3 1)) 1)  (= (readyToReceive (- 3 1)) 1)) 
(and
(= (msgBuffer 3) 1) 
(= (readyToReceive 3) 1) 
(= (messageReceived 3) 1) 
(= (waitForAck 3) (waitForAck (- 3 1))) 
(= (ackSent 3) (ackSent (- 3 1))) 
(= (ackBuffer 3) (ackBuffer (- 3 1))) 
(= (ackReceived 3) (ackReceived (- 3 1))) 
(= (readyToSend 3) (readyToSend (- 3 1))) 
)
 (ite (= (messageReceived (- 3 1)) 1) 
(and
(= (messageReceived 3) 1) 
(= (ackSent 3) 1) 
(= (ackBuffer 3) 1) 
(= (waitForAck 3) (waitForAck (- 3 1))) 
(= (msgBuffer 3) (msgBuffer (- 3 1))) 
(= (readyToReceive 3) (readyToReceive (- 3 1))) 
(= (ackReceived 3) (ackReceived (- 3 1))) 
(= (readyToSend 3) (readyToSend (- 3 1))) 
)
 (ite (and (= (waitForAck (- 3 1)) 1)  (= (ackBuffer (- 3 1)) 1)) 
(and
(= (waitForAck 3) 1) 
(= (ackBuffer 3) 1) 
(= (ackReceived 3) 1) 
(= (messageReceived 3) (messageReceived (- 3 1))) 
(= (msgBuffer 3) (msgBuffer (- 3 1))) 
(= (readyToReceive 3) (readyToReceive (- 3 1))) 
(= (ackSent 3) (ackSent (- 3 1))) 
(= (readyToSend 3) (readyToSend (- 3 1))) 
)
 (ite (= (ackSent (- 3 1)) 1) 
(and
(= (ackSent 3) 1) 
(= (readyToReceive 3) 1) 
(= (waitForAck 3) (waitForAck (- 3 1))) 
(= (messageReceived 3) (messageReceived (- 3 1))) 
(= (msgBuffer 3) (msgBuffer (- 3 1))) 
(= (ackReceived 3) (ackReceived (- 3 1))) 
(= (ackBuffer 3) (ackBuffer (- 3 1))) 
(= (readyToSend 3) (readyToSend (- 3 1))) 
)
 (ite (= (ackReceived (- 3 1)) 1) 
(and
(= (ackReceived 3) 1) 
(= (readyToSend 3) 1) 
(= (waitForAck 3) (waitForAck (- 3 1))) 
(= (messageReceived 3) (messageReceived (- 3 1))) 
(= (msgBuffer 3) (msgBuffer (- 3 1))) 
(= (ackSent 3) (ackSent (- 3 1))) 
(= (ackBuffer 3) (ackBuffer (- 3 1))) 
(= (readyToReceive 3) (readyToReceive (- 3 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 4 1)) 1) 
(and
(= (waitForAck 4) 1) 
(= (msgBuffer 4) 1) 
(= (readyToSend 4) 1) 
(= (messageReceived 4) (messageReceived (- 4 1))) 
(= (ackSent 4) (ackSent (- 4 1))) 
(= (ackBuffer 4) (ackBuffer (- 4 1))) 
(= (ackReceived 4) (ackReceived (- 4 1))) 
(= (readyToReceive 4) (readyToReceive (- 4 1))) 
)
 (ite (and (= (msgBuffer (- 4 1)) 1)  (= (readyToReceive (- 4 1)) 1)) 
(and
(= (msgBuffer 4) 1) 
(= (readyToReceive 4) 1) 
(= (messageReceived 4) 1) 
(= (waitForAck 4) (waitForAck (- 4 1))) 
(= (ackSent 4) (ackSent (- 4 1))) 
(= (ackBuffer 4) (ackBuffer (- 4 1))) 
(= (ackReceived 4) (ackReceived (- 4 1))) 
(= (readyToSend 4) (readyToSend (- 4 1))) 
)
 (ite (= (messageReceived (- 4 1)) 1) 
(and
(= (messageReceived 4) 1) 
(= (ackSent 4) 1) 
(= (ackBuffer 4) 1) 
(= (waitForAck 4) (waitForAck (- 4 1))) 
(= (msgBuffer 4) (msgBuffer (- 4 1))) 
(= (readyToReceive 4) (readyToReceive (- 4 1))) 
(= (ackReceived 4) (ackReceived (- 4 1))) 
(= (readyToSend 4) (readyToSend (- 4 1))) 
)
 (ite (and (= (waitForAck (- 4 1)) 1)  (= (ackBuffer (- 4 1)) 1)) 
(and
(= (waitForAck 4) 1) 
(= (ackBuffer 4) 1) 
(= (ackReceived 4) 1) 
(= (messageReceived 4) (messageReceived (- 4 1))) 
(= (msgBuffer 4) (msgBuffer (- 4 1))) 
(= (readyToReceive 4) (readyToReceive (- 4 1))) 
(= (ackSent 4) (ackSent (- 4 1))) 
(= (readyToSend 4) (readyToSend (- 4 1))) 
)
 (ite (= (ackSent (- 4 1)) 1) 
(and
(= (ackSent 4) 1) 
(= (readyToReceive 4) 1) 
(= (waitForAck 4) (waitForAck (- 4 1))) 
(= (messageReceived 4) (messageReceived (- 4 1))) 
(= (msgBuffer 4) (msgBuffer (- 4 1))) 
(= (ackReceived 4) (ackReceived (- 4 1))) 
(= (ackBuffer 4) (ackBuffer (- 4 1))) 
(= (readyToSend 4) (readyToSend (- 4 1))) 
)
 (ite (= (ackReceived (- 4 1)) 1) 
(and
(= (ackReceived 4) 1) 
(= (readyToSend 4) 1) 
(= (waitForAck 4) (waitForAck (- 4 1))) 
(= (messageReceived 4) (messageReceived (- 4 1))) 
(= (msgBuffer 4) (msgBuffer (- 4 1))) 
(= (ackSent 4) (ackSent (- 4 1))) 
(= (ackBuffer 4) (ackBuffer (- 4 1))) 
(= (readyToReceive 4) (readyToReceive (- 4 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 5 1)) 1) 
(and
(= (waitForAck 5) 1) 
(= (msgBuffer 5) 1) 
(= (readyToSend 5) 1) 
(= (messageReceived 5) (messageReceived (- 5 1))) 
(= (ackSent 5) (ackSent (- 5 1))) 
(= (ackBuffer 5) (ackBuffer (- 5 1))) 
(= (ackReceived 5) (ackReceived (- 5 1))) 
(= (readyToReceive 5) (readyToReceive (- 5 1))) 
)
 (ite (and (= (msgBuffer (- 5 1)) 1)  (= (readyToReceive (- 5 1)) 1)) 
(and
(= (msgBuffer 5) 1) 
(= (readyToReceive 5) 1) 
(= (messageReceived 5) 1) 
(= (waitForAck 5) (waitForAck (- 5 1))) 
(= (ackSent 5) (ackSent (- 5 1))) 
(= (ackBuffer 5) (ackBuffer (- 5 1))) 
(= (ackReceived 5) (ackReceived (- 5 1))) 
(= (readyToSend 5) (readyToSend (- 5 1))) 
)
 (ite (= (messageReceived (- 5 1)) 1) 
(and
(= (messageReceived 5) 1) 
(= (ackSent 5) 1) 
(= (ackBuffer 5) 1) 
(= (waitForAck 5) (waitForAck (- 5 1))) 
(= (msgBuffer 5) (msgBuffer (- 5 1))) 
(= (readyToReceive 5) (readyToReceive (- 5 1))) 
(= (ackReceived 5) (ackReceived (- 5 1))) 
(= (readyToSend 5) (readyToSend (- 5 1))) 
)
 (ite (and (= (waitForAck (- 5 1)) 1)  (= (ackBuffer (- 5 1)) 1)) 
(and
(= (waitForAck 5) 1) 
(= (ackBuffer 5) 1) 
(= (ackReceived 5) 1) 
(= (messageReceived 5) (messageReceived (- 5 1))) 
(= (msgBuffer 5) (msgBuffer (- 5 1))) 
(= (readyToReceive 5) (readyToReceive (- 5 1))) 
(= (ackSent 5) (ackSent (- 5 1))) 
(= (readyToSend 5) (readyToSend (- 5 1))) 
)
 (ite (= (ackSent (- 5 1)) 1) 
(and
(= (ackSent 5) 1) 
(= (readyToReceive 5) 1) 
(= (waitForAck 5) (waitForAck (- 5 1))) 
(= (messageReceived 5) (messageReceived (- 5 1))) 
(= (msgBuffer 5) (msgBuffer (- 5 1))) 
(= (ackReceived 5) (ackReceived (- 5 1))) 
(= (ackBuffer 5) (ackBuffer (- 5 1))) 
(= (readyToSend 5) (readyToSend (- 5 1))) 
)
 (ite (= (ackReceived (- 5 1)) 1) 
(and
(= (ackReceived 5) 1) 
(= (readyToSend 5) 1) 
(= (waitForAck 5) (waitForAck (- 5 1))) 
(= (messageReceived 5) (messageReceived (- 5 1))) 
(= (msgBuffer 5) (msgBuffer (- 5 1))) 
(= (ackSent 5) (ackSent (- 5 1))) 
(= (ackBuffer 5) (ackBuffer (- 5 1))) 
(= (readyToReceive 5) (readyToReceive (- 5 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 6 1)) 1) 
(and
(= (waitForAck 6) 1) 
(= (msgBuffer 6) 1) 
(= (readyToSend 6) 1) 
(= (messageReceived 6) (messageReceived (- 6 1))) 
(= (ackSent 6) (ackSent (- 6 1))) 
(= (ackBuffer 6) (ackBuffer (- 6 1))) 
(= (ackReceived 6) (ackReceived (- 6 1))) 
(= (readyToReceive 6) (readyToReceive (- 6 1))) 
)
 (ite (and (= (msgBuffer (- 6 1)) 1)  (= (readyToReceive (- 6 1)) 1)) 
(and
(= (msgBuffer 6) 1) 
(= (readyToReceive 6) 1) 
(= (messageReceived 6) 1) 
(= (waitForAck 6) (waitForAck (- 6 1))) 
(= (ackSent 6) (ackSent (- 6 1))) 
(= (ackBuffer 6) (ackBuffer (- 6 1))) 
(= (ackReceived 6) (ackReceived (- 6 1))) 
(= (readyToSend 6) (readyToSend (- 6 1))) 
)
 (ite (= (messageReceived (- 6 1)) 1) 
(and
(= (messageReceived 6) 1) 
(= (ackSent 6) 1) 
(= (ackBuffer 6) 1) 
(= (waitForAck 6) (waitForAck (- 6 1))) 
(= (msgBuffer 6) (msgBuffer (- 6 1))) 
(= (readyToReceive 6) (readyToReceive (- 6 1))) 
(= (ackReceived 6) (ackReceived (- 6 1))) 
(= (readyToSend 6) (readyToSend (- 6 1))) 
)
 (ite (and (= (waitForAck (- 6 1)) 1)  (= (ackBuffer (- 6 1)) 1)) 
(and
(= (waitForAck 6) 1) 
(= (ackBuffer 6) 1) 
(= (ackReceived 6) 1) 
(= (messageReceived 6) (messageReceived (- 6 1))) 
(= (msgBuffer 6) (msgBuffer (- 6 1))) 
(= (readyToReceive 6) (readyToReceive (- 6 1))) 
(= (ackSent 6) (ackSent (- 6 1))) 
(= (readyToSend 6) (readyToSend (- 6 1))) 
)
 (ite (= (ackSent (- 6 1)) 1) 
(and
(= (ackSent 6) 1) 
(= (readyToReceive 6) 1) 
(= (waitForAck 6) (waitForAck (- 6 1))) 
(= (messageReceived 6) (messageReceived (- 6 1))) 
(= (msgBuffer 6) (msgBuffer (- 6 1))) 
(= (ackReceived 6) (ackReceived (- 6 1))) 
(= (ackBuffer 6) (ackBuffer (- 6 1))) 
(= (readyToSend 6) (readyToSend (- 6 1))) 
)
 (ite (= (ackReceived (- 6 1)) 1) 
(and
(= (ackReceived 6) 1) 
(= (readyToSend 6) 1) 
(= (waitForAck 6) (waitForAck (- 6 1))) 
(= (messageReceived 6) (messageReceived (- 6 1))) 
(= (msgBuffer 6) (msgBuffer (- 6 1))) 
(= (ackSent 6) (ackSent (- 6 1))) 
(= (ackBuffer 6) (ackBuffer (- 6 1))) 
(= (readyToReceive 6) (readyToReceive (- 6 1))) 
)
(= true false))))))))
(assert  (ite (= (readyToSend (- 7 1)) 1) 
(and
(= (waitForAck 7) 1) 
(= (msgBuffer 7) 1) 
(= (readyToSend 7) 1) 
(= (messageReceived 7) (messageReceived (- 7 1))) 
(= (ackSent 7) (ackSent (- 7 1))) 
(= (ackBuffer 7) (ackBuffer (- 7 1))) 
(= (ackReceived 7) (ackReceived (- 7 1))) 
(= (readyToReceive 7) (readyToReceive (- 7 1))) 
)
 (ite (and (= (msgBuffer (- 7 1)) 1)  (= (readyToReceive (- 7 1)) 1)) 
(and
(= (msgBuffer 7) 1) 
(= (readyToReceive 7) 1) 
(= (messageReceived 7) 1) 
(= (waitForAck 7) (waitForAck (- 7 1))) 
(= (ackSent 7) (ackSent (- 7 1))) 
(= (ackBuffer 7) (ackBuffer (- 7 1))) 
(= (ackReceived 7) (ackReceived (- 7 1))) 
(= (readyToSend 7) (readyToSend (- 7 1))) 
)
 (ite (= (messageReceived (- 7 1)) 1) 
(and
(= (messageReceived 7) 1) 
(= (ackSent 7) 1) 
(= (ackBuffer 7) 1) 
(= (waitForAck 7) (waitForAck (- 7 1))) 
(= (msgBuffer 7) (msgBuffer (- 7 1))) 
(= (readyToReceive 7) (readyToReceive (- 7 1))) 
(= (ackReceived 7) (ackReceived (- 7 1))) 
(= (readyToSend 7) (readyToSend (- 7 1))) 
)
 (ite (and (= (waitForAck (- 7 1)) 1)  (= (ackBuffer (- 7 1)) 1)) 
(and
(= (waitForAck 7) 1) 
(= (ackBuffer 7) 1) 
(= (ackReceived 7) 1) 
(= (messageReceived 7) (messageReceived (- 7 1))) 
(= (msgBuffer 7) (msgBuffer (- 7 1))) 
(= (readyToReceive 7) (readyToReceive (- 7 1))) 
(= (ackSent 7) (ackSent (- 7 1))) 
(= (readyToSend 7) (readyToSend (- 7 1))) 
)
 (ite (= (ackSent (- 7 1)) 1) 
(and
(= (ackSent 7) 1) 
(= (readyToReceive 7) 1) 
(= (waitForAck 7) (waitForAck (- 7 1))) 
(= (messageReceived 7) (messageReceived (- 7 1))) 
(= (msgBuffer 7) (msgBuffer (- 7 1))) 
(= (ackReceived 7) (ackReceived (- 7 1))) 
(= (ackBuffer 7) (ackBuffer (- 7 1))) 
(= (readyToSend 7) (readyToSend (- 7 1))) 
)
 (ite (= (ackReceived (- 7 1)) 1) 
(and
(= (ackReceived 7) 1) 
(= (readyToSend 7) 1) 
(= (waitForAck 7) (waitForAck (- 7 1))) 
(= (messageReceived 7) (messageReceived (- 7 1))) 
(= (msgBuffer 7) (msgBuffer (- 7 1))) 
(= (ackSent 7) (ackSent (- 7 1))) 
(= (ackBuffer 7) (ackBuffer (- 7 1))) 
(= (readyToReceive 7) (readyToReceive (- 7 1))) 
)
(= true false))))))))

(check-sat)
(get-model)