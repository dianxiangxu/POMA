...
public class ROC_Solver extends MutantTester {
	static Graph g;
	public ROC_Solver(String testMethod, Graph graph, Prohibitions prohibitions, String obligationPath, List<AccessRequest> arList) throws GraphDoesNotMatchTestSuitException {
		...
		for (Rule rule : rules) {
			...
			for (Action actionToDelete : actions) {
				...
				for (int j = 0; j < actions.size(); j++) {
					...
					if (index == j) {
						//remove condition of action, and save condition for revert condition after mutation
