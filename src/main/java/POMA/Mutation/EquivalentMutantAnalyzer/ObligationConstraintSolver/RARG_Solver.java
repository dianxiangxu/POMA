...
public class RARG_Solver extends MutantTester {
	static Graph g;
	public RARG_Solver(String testMethod, Graph graph, Prohibitions prohibitions, String obligationPath, List<AccessRequest> arList) throws GraphDoesNotMatchTestSuitException {
	...
	public void init() throws Exception {
		...
		for (Rule rule : rules) {
			...
			for (Action actionToChange : actions) {
				...
				for (String accessRight : allOps) {
					operations.clear();
					for (String op : allOps)
						operations.add(op);
					operations.remove(accessRight);
					...
					PConstraint = Utils.getPropagationConstraintRARG(subject.getName(), target.getName(), accessRight);
