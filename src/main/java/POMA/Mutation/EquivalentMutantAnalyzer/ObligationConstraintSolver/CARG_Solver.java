...
public class CARG_Solver extends MutantTester {
	static Graph g;
	public CARG_Solver(String testMethod, Graph graph, Prohibitions prohibitions, String obligationPath, List<AccessRequest> arList) throws GraphDoesNotMatchTestSuitException {
	...
	public void init() throws Exception {
		...
		for (Rule rule : rules) {
			...
			for (Action actionToChange : actions) {
				...
				for (String oldAr : allOps) {
				for (String accessRight : Utils.getAllAccessRights()) {
					if (oldAr.equals(accessRight))
						continue;
					...
					PConstraint = Utils.getPropagationConstraintCARG(subject.getName(), target.getName(), oldAr, accessRight);
