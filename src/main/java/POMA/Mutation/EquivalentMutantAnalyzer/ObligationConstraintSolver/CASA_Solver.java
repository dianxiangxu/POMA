...
public class CASA_Solver extends MutantTester {
	static Graph g;
	public CASA_Solver(String testMethod, Graph graph, Prohibitions prohibitions, String obligationPath, List<AccessRequest> arList) throws GraphDoesNotMatchTestSuitException {
		...
		for (Rule rule : rules) {
			...
			for (Action actionToChange : actions) {
				...
				for (Assignment as : assignments) {
					...
					for (Node newWhat : UAsOAs) {
						if (newWhat.getName().equals(what.getName()))
							continue;
						if (!newWhat.getType().toString().equals(what.getType()))
							continue;
						if (Utils.isContained(oldWhere.getName(), newWhat.getName(), graph))
							continue;
						what2 = new EvrNode(newWhat.getName(), newWhat.getType().toString(), newWhat.getProperties());
						
						System.out.println("(CASA_Solver) " + ruleLabel + "|actionIndex:" + index);
						System.out.println("Old What:" + what.getName() + "|" + what.getType());
						System.out.println("Where:" + as.getWhere().getName() + "|" + as.getWhere().getType());
						System.out.println("New what:" + what2.getName() + "|" + what2.getType());
						Assignment assignment = new Assignment(what2, oldWhere);
...
//						}
						...
						PConstraint = Utils.getPropagationConstraintCASA(what.getName(), oldWhere.getName(), what2.getName());
