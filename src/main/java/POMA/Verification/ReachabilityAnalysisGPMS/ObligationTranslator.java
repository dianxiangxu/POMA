...
public class ObligationTranslator {
	...
	private List<String> customProcessedObligations;
	Obligation obligation;
	HashMap<String, Integer> mapOfIDs;
	Plugin plugin;

	public ObligationTranslator(String pathToObligations) {
		...
		translateObligationEventLabels();
	}

	void executeCustomPlugins(Plugin... plugins) {
		if (plugins.length > 0) {
			for (Plugin plugin : plugins) {
				plugin.setObligations(obligation);
				plugin.setMapOfIds(mapOfIDs);
				plugin.exec();
			}
			this.plugin = plugins[0];
			this.customProcessedObligations = plugin.getProcessedObligation();
		}
	}
	...
	void findAllAbsentElements() {
		for (Rule rule : obligation.getRules()) {
			...
			listOfAddedAssociations.add(new AssociationRelation(rule.getEventPattern().getSubject().getAnyUser().get(0),
					new OperationSet((Collection<String>) rule.getEventPattern().getOperations()),
					rule.getEventPattern().getTarget().getPolicyElements().get(0).getName()));
		}
	}
	...
	String translateObligationEvents(int k) {
		...
		for (Rule r : obligation.getRules()) {
			...
			if (plugin.getStateResponseConditionVariables().containsKey(r.getLabel() + "0")) {
				for (int i = 0; i < 10; i++) {
					if (!plugin.getStateResponseConditionVariables().containsKey(r.getLabel() + i)) {
						break;
					}
					sb.append("(assert \r\n" + "(xor \r\n" + "(= (" + (r.getLabel() + i) + " " + (k - 1) + ") 0) \r\n"
							+ "(and (member (mkTuple " + subjectID + " " + arID + " " + targetID + ") (ASSOC* "
							+ (k - 1) + "))  " + "" + " (= (" + (r.getLabel() + i) + " " + (k - 1) + ") 1))\r\n"
							+ ")\r\n" + ")				\r\n");
					ruleLabels.add(r.getLabel() + i);
				}
			} else if (customProcessedObligations.contains(r.getLabel())) {
				processPluginEventCondition(r, k, sb);
			} else {
	...
	void processPluginEventCondition(Rule r, int k, StringBuilder sb) {
		String condition = plugin.getEventConditions().get(r.getLabel());
		String obligationLabel = r.getLabel();
		String subject = r.getEventPattern().getSubject().getAnyUser().get(0);
		String ar = r.getEventPattern().getOperations().get(0);
		String target = r.getEventPattern().getTarget().getPolicyElements().get(0).getName();
		...
		if (condition == null) {
			for (int i = 0; i < 10; i++) {
				obligationLabel = r.getLabel() + i;
				condition = plugin.getEventConditions().get(obligationLabel);
				if (condition == null) {
					return;
				}
	...
	private void translateObligationEventLabels() {
	...
	String processCustomStateVariables(int k) {
		StringBuilder sb_custom_variables = new StringBuilder();
		for (Map.Entry<String, List<String>> entry : plugin.getStateResponseConditionVariables().entrySet()) {
		...
		return sb_custom_variables.toString();
	}
	...
	String processActions(int k) {
		...
		sb_associations.append(System.lineSeparator());
		for (Rule rule : obligation.getRules()) {
			if (customProcessedObligations.contains(rule.getLabel())) {
				List<Action> pluginActions = plugin.getObligationEffects().get(rule.getLabel());
				if (pluginActions != null) {
					this.groupActions(plugin.getObligationEffects().get(rule.getLabel()));
				} else {
					for (int i = 0; i < 10; i++) {
						String label = rule.getLabel() + i;
						String condition = "";

						if (plugin.getObligationEffects().get(label) != null) {
							this.groupActions(plugin.getObligationEffects().get(label));
						} else if (plugin.getStateResponseConditionVariables().get(label) != null) {
						...
						if (grantGroupActions.size() > 0) {
							sb_associations.append("(and " + condition + " (= (" + label + " " + (k - 1) + ") 1)");
							sb_associations.append(System.lineSeparator());
						...
						if (assignGroupActions.size() > 0) {
							sb_assignments_flatten
									.append("(and " + condition + "  (= (" + label + " " + (k - 1) + ") 1)");
							sb_assignments_flatten.append(System.lineSeparator());
