...
public class ImplicitAssignPredicate implements IPredicate{
	
	List<ITerm >tuple = new ArrayList<ITerm>();
	
	public ImplicitAssignPredicate() {
	}
	...
	public String toString() {
		return "ImplicitAssignPredicate [tuple=" + tuple + "]";
	}

	public String toSMT() throws Exception {
		if (tuple.size() != 2) {
			throw new Exception(
					"Incorrect ImplicitAssignPredicate predictate format. Please use the following format: ImplicitAssignPredicate(ancestor, descendant)");
		}
		String smtlibv2Code = "";
		...
		smtlibv2Code += "(set.member (tuple " + aSpec + " " + dSpec + " ) (set.minus (ASSIGN* " + "{(k + 1)}"
				+ ")(ASSIGN " + "{(k + 1)}" + ")))";
