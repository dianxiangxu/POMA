package CaseStudies.Bank.customFunctions;
...
public class HasPropertiesExecutor implements FunctionExecutor {
	@Override
	...
	public Boolean exec(EventContext eventCtx, String user, String process, PDP pdp, Function function,
		...
		Map<String, String> mapProperties = node.getProperties();
		for (String key : props.keySet()) {
			if (mapProperties.containsKey(key)) {
				if (mapProperties.get(key).equals(props.get(key))) {
					return true;
				} else {
					return false;
				}
