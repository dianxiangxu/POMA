package CaseStudies.gpms.policy.customFunctions;
...
public class GetAncestorsInPCExecutor implements FunctionExecutor {
	...
	@Override
	public List<String> exec(EventContext eventCtx, String user, String process, PDP pdp, Function function,
		...
		Node parentUA = functionEvaluator.evalNode(eventCtx, user, process, pdp, argFunction1);
		...
		for (String child : pdp.getPAP().getGraphPAP().getChildren(parentUA.getName())) {
			Set<String> parents = pdp.getPAP().getGraphPAP().getParents(child);
			...
			for (String parent : parents) {
				...
				if (count == levelOfAncestory && isContainedIn) {
					listToReturn.add(parentForChild.getName());
			...
			while (true) {
				count++;
				if(parentVariable.equals("")) break;
				...
				if (count == levelOfAncestory && isContainedIn) {
					...
					listToReturn.add(pdp.getPAP().getGraphPAP().getNode(ancestor).getName());
				} else if (isContainedIn) {
					parentVariable = ancestor;
					continue;
				} else {
					throw new PMException(
							"The assumption was that one parent can only have one child in this hierarchy");
				}
			}
